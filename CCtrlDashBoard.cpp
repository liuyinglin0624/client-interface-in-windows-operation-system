...
CCtrlDashBoard::CCtrlDashBoard(QWidget *parent,bool bFlag, StyleType type) :
    QWidget(parent),
    m_StyleType(type)
{
    this->m_bType = bFlag;
...
void CCtrlDashBoard::drawBackGround(QPainter *painter, qreal hlafWidth)
{
    ...
    if(this->m_bType)
        ...
        painter->drawText(startX+90,startX+170,"°/S");
    painter->restore();
...
void CCtrlDashBoard::drawIndicatorBar(QPainter *painter, qreal hlafWidth)
{
    ...
    if(m_DashValue>0){
        qreal tSteps = (m_MaxValue - m_MinValue)/m_DashNum; //相乘后的值是分的份数
        qreal angleStep = 1.0*(360.0-m_StartAngle - m_EndAngle) / tSteps; //每一个份数的角度
        degRotate = angleStep*tSteps*(m_DashValue/180.0)+angleStep;
